...
#| echo: false
#| message: false
#| fig-cap: "Average change in pass rates in standardised tests by the percent of hybrid learning"

# Prepare data for virtual learning model in MATH
sorted_math_hybrid <- 
  math_data |>
  mutate(hybrid_group = cut(share_hybrid,
                              breaks = c(0, 25, 50, 75, 100),
                              labels = c("0-25%", "25-50%", "50-75%", "75-100%"),
                              include.lowest = TRUE))
average_pass_rates <- 
  sorted_math_hybrid |>
  group_by(year, hybrid_group) |>
  summarise(average_pass_rate = mean(pass_rate, na.rm = TRUE))
...
# Plot the resultant graph
hybrid_math <-
  ggplot(long_filtered_test_data, aes(x = change_in_pass_rate, y = hybrid_group, color = factor(year))) +
  geom_point(size = 4) + 
  geom_vline(xintercept = 0, color = "darkgrey") +
  labs(x = "Change Rate in Math (%)", y = "Percent Hybrid") +
  scale_color_discrete(labels = c("Spring 2017", "Spring 2018", "Spring 2019", "Spring 2021")) +
  labs(color = "Year Category") +
  theme_minimal()
...
hybrid_ela <-
  ...
  geom_point(size = 4) + 
  geom_vline(xintercept = 0, color = "darkgrey") +
  labs(x = "Change Rate in ELA (%)", y = "Percent Hybrid") +
  scale_color_discrete(labels = c("Spring 2017", "Spring 2018", "Spring 2019", "Spring 2021")) +
  labs(color = "Year Category") +
  theme_minimal()
